...
float PointDistance (CGPoint p1, CGPoint p2)
{
    return sqrtf((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y));
}


PixelRGB HSBToRGB (float h, float s, float v)
{
    h *= 6.0f;
    int i = floorf(h);
    float f = h - (float)i;
...
- (PixelRGB)colorAtPoint:(CGPoint)point
{
    ...
    float dist = PointDistance(point, CGPointMake(center.x, center.y));
    ...
    return HSBToRGB(hue, sat, _brightness);
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (_continuous)
    {
        [_delegate colorWheelDidChangeColor:[self currentColor]];
    }
}
...
- (void)setTouchPoint:(CGPoint)point
{
    ...
    if (PointDistance(center, point) < _radius)
    {
        _touchPoint = point;
    }
    else
